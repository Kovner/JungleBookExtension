...
function applyShapeFilter(event){
    ...
    tableau.extensions.dashboardContent.dashboard.worksheets[worksheetIndex].applyFilterAsync(
...
function updateFilterFields(e) {
    ...
    tableau.extensions.dashboardContent.dashboard.worksheets[worksheetIndex].getFiltersAsync().then( 
        function(filters) {
...
function saveAll() {
    ...
    tableau.extensions.settings.set('imageUrl', getElem('image-url').value);
    tableau.extensions.settings.set('sheetName', getElem('sheet-names').value);
    tableau.extensions.settings.set('filterName', getElem('filter-fields').value);
    const polygons = getElem('svg-root').getElementsByTagNameNS("http://www.w3.org/2000/svg", 'polygon');
    for (let i=0; i<polygons.length; ++i) {
        tableau.extensions.settings.set('polygon' + i + "Value", polygons[i].getAttribute('title'));
        tableau.extensions.settings.set('polygon' + i + 'Points', polygons[i].getAttribute('points'));
    }
...
function onBodyLoad() {  
    ...
    tableau.extensions.initializeAsync().then( function() {
            const selectedSheet = tableau.extensions.settings.get('sheetName');
            let allSheets = tableau.extensions.dashboardContent.dashboard.worksheets;
            ...
            for( let i=0; i<100; i++) {
                ...
                const polyValue = tableau.extensions.settings.get('polygon' + i + 'Value');
                if (null != polyValue) {
                    const polyPoints = tableau.extensions.settings.get('polygon' + i + 'Points');
                    createSavedPolygon(polyPoints, polyValue, true);
                } else {
                    break;
                }
            }
            const imageUrl = tableau.extensions.settings.get('imageUrl');
            if (null != imageUrl) {
